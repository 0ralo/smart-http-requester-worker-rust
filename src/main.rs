...
use crate::schemas::{RabbitmqTask};
...
async fn init_pool(settings: &Settings) -> anyhow::Result<Pool<Postgres>> {
...
async fn init_rmq(settings: &Settings) -> anyhow::Result<Connection> {
    let uri = AMQPUri {
        scheme: AMQPScheme::AMQP,
        authority: AMQPAuthority {
            userinfo: AMQPUserInfo {
                username: settings.rabbitmq.user.clone(),
                password: settings.rabbitmq.password.clone(),
            },
            host: settings.rabbitmq.host.clone(),
            port: settings.rabbitmq.port,
        },
        vhost: settings.rabbitmq.vhost.clone(),
...
async fn main() -> anyhow::Result<()> {
    ...
    let rmq = init_rmq(&settings).await?;

    let channel = rmq.create_channel().await?;
    let mut consumer = channel
        .basic_consume(
            "tasks.queue".into(),
            settings.worker.name.into(),
            BasicConsumeOptions::default(),
            FieldTable::default(),
        )
        .await
        .context("Failed to create consumer from rmq")?;
...
async fn send_to_retry_or_failed(
    ...
    if attempt_count < max_attempts {
        ...
        channel
            ...
            .await
            .context("Failed to publish to retry_queue")?;
...
async fn process_message(
    ...
    let data = std::str::from_utf8(message)
        .context("Failed to convert message to string")?;
    ...
    match result {
        Ok(r) => {
            sqlx::query!("update tasks set status='done', attempt_count = attempt_count + 1, updated_at=now(), result=jsonb_build_object('result', $1::text) where id = $2", r, &task_o.task_id)
                .execute(&mut *tx)
                .await
                .context("Failed to update task in ok section")?;
            info!("Task finished succesfully")
        }
        Err(e) => {
            if let Err(e) = send_to_retry_or_failed(channel, &task_o.task_id, t.attempt_count + 1, t.max_attempts, e.to_string()).await {
                sqlx::query!("update tasks set updated_at=now(), status = 'failed', attempt_count = attempt_count + 1 where id = $1", &task_o.task_id)
                    .execute(&mut *tx)
                    .await
                    .context("Failed to update task in err section status=failed")?;
                ...
                sqlx::query!("update tasks set updated_at=now(), attempt_count = attempt_count + 1 where id = $1", &task_o.task_id)
                    .execute(&mut *tx)
                    .await
                    .context("Failed to update task in err section cont += 1")?;
            }
